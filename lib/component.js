...
Component.helpers = function (helpers) {
  var helperMap = defaultValue(this, '_helpers', {});

  for (var key in helpers) {
    check(helpers[key], Function);
    helperMap[key] = (function (key) {
      return function () {
        var cmp = Component.findFirstComponent(Blaze.getView());
        check(cmp, Iron.Component);
        return helpers[key].apply(cmp, arguments);
      };
    }(key));
  }
