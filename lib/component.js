...
var findFirstComponent = function (view) {
  assert(view instanceof Blaze.View, "view must be a Blaze.View");

  while (view) {
    if (typeof view.__component__ !== 'undefined')
      return view.__component__;
    else
      view = view.parentView;
  }
...
Component.prototype.createView = function (options) {
  ...
  */
  ...
  view.onViewCreated(function () {
    if (self.constructor.created) {
      self.constructor.created.call(self, view.templateInstance());
    }
  });

  view.onViewReady(function () {
    if (self.constructor.rendered) {
      self.constructor.rendered.call(self, view.templateInstance());
    }
  });

  view.onViewDestroyed(function () {
    if (self.constructor.destroyed) {
      self.constructor.destroyed.call(self, view.templateInstance());
    }
  });
  ...
  // This puts the component into the Blaze.prototype.lookup chain. The logic
  // for this is defined in iron:dynamic-template.
  DynamicTemplate.registerLookupHost(view, this);
...
Component.prototype._renderView = function () {
  var tmpl = this.render();

  if (tmpl.template) {
    _.extend(tmpl.template, this.constructor._helpers);
  }
...
Component.lookup = function (view, prop) {
  ...
  while (view) {
    if ((host = view.template) && host[prop])
      return host[prop];
    else if ((host = view.__lookupHost__) && host[prop])
      return (typeof host[prop] === 'function') ? host[prop].bind(host) : host[prop];
    else
      view = view.parentView;
  }
